...
// === Filter UI Initialization & Events ===
...
function initFilterToggles() {
  filterToggle.addEventListener("click", () => {
    ...
    if (isOpening) {
      ...
      // Overflow visible ensures calendar drop breaks bounds gracefully
      setTimeout(() => {
  ...
  scopePills.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (filterState.scope[type] && !filterState.scope[otherType]) {
        return; 
      }
  ...
  clearFiltersBtn.addEventListener("click", () => {
    ...
    document.querySelectorAll(".pill.tag-pill").forEach(btn => btn.classList.remove("active"));
...
function initTagsFilter() {
  ...
  allTags.forEach(tag => {
    const hex = TAG_COLORS[tagToSlug(tag)] || "#6B7280";
...
function renderCalendar() {
  ...
  for (let i = 1; i <= totalDays; i++) {
    ...
    wrap.className = "calendar-day-wrap";
...
function initDateControls() {
  ...
  document.addEventListener("click", (e) => {
    if (!e.target.closest(".date-range") && activeDateField !== null) {
      calendarDropdown.style.display = "none";
      activeDateField = null;
    }
  });
